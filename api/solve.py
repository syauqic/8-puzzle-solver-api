...
def convertFrontEndToMoves(bfsList) -> dict:
 ...
 # ### PERUBAHAN: Menentukan prevMove dari state di index 0 ###
...
def convertToFrontEnd(steps_from_bfs: list) -> list:
 """
 MENGGANTIKAN FUNGSI backTrack.
 Mengambil path solusi lengkap dari langkah terakhir yang mencapai goal, 
 memastikan urutan langkah runtut DAN menyertakan nilai heuristik.
 """
 frontEndList = []

 # 1. Cek apakah BFS menemukan solusi
 if not steps_from_bfs or steps_from_bfs[-1]["heuristic"] != 0:
  return []

 # 2. Langkah terakhir berisi path lengkap ke goal state
 final_step = steps_from_bfs[-1]
 
 # ### PERUBAHAN: Membuat daftar semua node di jalur solusi yang sudah dikunjungi ###
 ...
 # Iterasi mundur dari Goal state menggunakan path (list of parent tuples)
 # Pastikan goal state sendiri masuk ke dalam daftar dengan nilai heuristiknya
 path_info.append({
  "state": current_state_tuple,
  "heuristic": final_step["heuristic"]
 })
 
 # Ambil semua parent state (path)
 goalStateTuple = current_state_tuple
 path_tuples = final_step["path"] 
 
 # Kita perlu menghitung H untuk setiap parent/intermediate state
 # NOTE: Karena BFS hanya menyimpan H dari node yang diekspansi, kita harus hitung ulang H
 # atau jika Anda punya daftar semua node yang dieksplorasi (explored_list), 
 # Anda bisa mencari H di sana. Untuk penyederhanaan, kita hitung ulang.
 # Kita asumsikan goalState yang digunakan adalah goalState default (1,2,3/4,5,6/7,8,0)
 goalState = [ [1, 2, 3], [4, 5, 6], [7, 8, 0] ]

 for state_tuple in reversed(path_tuples):
  state_matrix = tupleToState(state_tuple)
  h_val = countHeuristic(state_matrix, goalState)
  path_info.append({
   "state": state_tuple,
   "heuristic": h_val
  })

 # Path Info sekarang urut terbalik (Goal -> Start). Balikkan urutan.
 path_info.reverse() 
 # ##########################################################

 # 3. Konversi path menjadi format list step frontend
 for step_data in path_info:
  step_entry = {
   "state": step_data["state"],
   "heuristic": step_data["heuristic"], # ### PERUBAHAN: Tambahkan Heuristik ###
   # frontEndPath hanya berisi state tujuan langkah ini (untuk animasi 1 langkah)
   "frontEndPath": [step_data["state"]] 
  }
...
def BestFirstSearch(initialState, goalState) -> dict:
 ...
 # ### PERUBAHAN: Simpan daftar semua node yang dieksplorasi (untuk visualisasi pohon) ###
 # explored_nodes = [] # Hapus karena tidak digunakan untuk saat ini.
 # #######################################################################################
 ...
 frontier.append({
  ...
  "path": []
  # ### PERUBAHAN: Tambahkan 'depth' atau 'g' jika menggunakan A* ###
 })
 ...
 while len(frontier) > 0:
  ...
  # ### PERUBAHAN: Simpan state sebelum lanjut ke node tetangga (untuk pohon) ###
  # explored_nodes.append(currentState) # Hapus karena tidak digunakan untuk saat ini.
  # #########################################################################
 ...
 return {
  ...
  # ### PERUBAHAN: Tambahkan heuristik awal untuk state awal ###
  "initialHeuristic": BestFirstSearch.initialHeuristic 
 }

# ### PERUBAHAN: Tambahkan atribut untuk menyimpan heuristik awal ###
BestFirstSearch.initialHeuristic = 0
...
def findMoves(currentStateDict) -> list[list[list[int]]]:
 # Fungsi findMoves tidak berubah
 # ... (lanjutan kode findMoves)
...
def getInitialState(choice) -> list[list[int]]:
 # Fungsi getInitialState tidak berubah
 # ... (lanjutan kode getInitialState)
 """
 Mengambil state awal berdasarkan pilihan angka.
 
 Jika choice=0, mode generate acak akan dijalankan.
 
 MODIFIKASI: Tambahkan kode yang bisa di-uncomment/comment untuk memastikan
 puzzle yang di-generate selalu SOLVABLE.
 """
 ...
 # ... (daftar initial_states tetap sama)
 initial_states = {
...
def handle_puzzle_request():
 ...
 try:
  ...
  if action == 'solve':
   ...
   # ### PERUBAHAN: Hitung dan simpan heuristik awal sebelum memanggil solver ###
   ...
   final_result = {
    ...
    # ### PERUBAHAN: Mengambil nilai Heuristik awal yang sudah disimpan ###
